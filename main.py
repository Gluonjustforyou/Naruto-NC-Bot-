  ...
  while True:
    try:
      ...
      if message_id is None:
        ...
        font = ImageFont.truetype("njnaruto.ttf",
                                  35)  # Schriftart und Größe anpassen
        ...
        draw.text(text_position,
                  current_status,
                  font=font,
                  fill=(255, 255, 255))  # Text auf Bild zeichnen
        draw.text(text_position, current_status, font=font,
                  fill="#E1EDFA")  # Text auf Bild zeichnen

        link_font = ImageFont.truetype("lastninja.ttf",
                                       35)  # Schriftart und Größe des Links
        link_bbox = draw.textbbox((0, 0), connectlink, font=link_font)
        link_text_width, link_text_height = link_bbox[2] - link_bbox[
          0], link_bbox[3] - link_bbox[1]
        link_position = (image.width - link_text_width - 10,
                         image.height - link_text_height - 10
                         )  # Position unten rechts
        draw.text(link_position,
                  connectlink,
                  font=link_font,
                  fill=(255, 255, 255))  # Link auf Bild zeichnen
        draw.text(link_position, connectlink, font=link_font,
                  fill="#E1EDFA")  # Link auf Bild zeichnen
        ...
        if current_status != previous_status:  # Überprüfe, ob sich der Serverstatus geändert hat
          ...
          link_font = ImageFont.truetype("lastninja.ttf",
                                         25)  # Schriftart und Größe des Links
          link_bbox = draw.textbbox((0, 0), connectlink, font=link_font)
          link_text_width, link_text_height = link_bbox[2] - link_bbox[
            0], link_bbox[3] - link_bbox[1]
          link_position = (image.width - link_text_width - 10,
...
def get_player_info(server_ip, server_port, rcon_password):
  # Verbindung zum GMod-Server herstellen
  rcon_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  rcon_socket.settimeout(1)

  try:
    rcon_socket.connect((server_ip, server_port))
  except socket.error as e:
    print(f"Fehler beim Verbinden zum Gmod-Server: {e}")
    return 0, []

  # RCON-Authentifizierung
  auth_command = f"rcon_password {rcon_password}\n"
  rcon_socket.sendall(auth_command.encode())

  # Spielerliste abrufen
  player_command = "rcon status\n"
  rcon_socket.sendall(player_command.encode())
  response = rcon_socket.recv(4096).decode()

  rcon_socket.close()

  # Spieleranzahl und Steam-Namen aus der Antwort extrahieren
  pattern = r"#\d+ \[U:1:\d+\] ([\w ]+)\n"
  matches = re.findall(pattern, response)
  player_count = len(matches)
  player_names = matches
